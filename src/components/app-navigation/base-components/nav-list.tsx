...
export const NavList = ({ activeUrl, items, className, collapsed }: NavListProps) => {
    const [open, setOpen] = useState(false);
    const activeItem = items.find((item) => item.href === activeUrl || item.items?.some((subItem) => subItem.href === activeUrl));
    const [currentItem, setCurrentItem] = useState(activeItem);

    return (
        <ul className={cx("mt-4 flex flex-col", collapsed ? "px-2" : "px-2 lg:px-4", className)}>
            {items.map((item, index) => {
                ...
                // Don't show collapsible items when collapsed
